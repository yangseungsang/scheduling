...
스케줄 템플릿은 `schedule/base.html`을 기준으로 Bootstrap, vendor 파일, schedule CSS/JS를 로드한다. 실행 템플릿은 `execution/base.html`과 별도 CSS/JS를 사용한다.
...
| `schedule-features.js` | 주말 토글, 일정 이동, 단순 블록, 큐 검색/그룹, hover 강조, 월간 더보기 |
...
| 진행/완료/불가 색상 | execution 상태 기반 표시 |
...
| 파일 | 역할 |
| --- | --- |
| `execution-app.js` | 실행 목록/일간 화면, 필터, 시작/일시정지/완료 액션 |
| `execution-detail.js` | 식별자 상세 실행 화면 |

실행 화면은 `/execution/api/list`로 스케줄에 배치된 식별자를 읽고, 각 식별자의 실행 레코드가 없으면 pending으로 표시한다. 타이머 버튼은 `/execution/api/start`, `/pause`, `/resume`, `/complete`를 호출한다.
...
6. 드래그/리사이즈 변경은 잠금, 장소 필터, 휴식 시간, 업무 종료 초과 규칙을 함께 검증한다.
