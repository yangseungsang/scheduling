...
storage adapter 기반 CRUD 연산을 제공하는 BaseRepository 클래스를 정의한다.
...
class BaseRepository:
    """스케줄 모델 레포지토리의 기본 클래스.

    하위 클래스에서 FILENAME, ID_PREFIX를 지정하면
    해당 storage backend에 대한 CRUD 기능을 자동으로 제공한다.
    ...
    def get_all(cls):
        ...
        Returns:
            list[dict]: storage에 저장된 전체 항목 리스트
        """
        return cls._storage().get_all(cls.FILENAME)
    ...
    def create(cls, data):
        ...
        items = storage.get_all(cls.FILENAME)
        if 'id' not in data or not data['id']:
            # ID가 없으면 접두사 기반으로 고유 ID 자동 생성
            data['id'] = storage.generate_id(cls.ID_PREFIX)
        items.append(data)
        storage.save_all(cls.FILENAME, items)
        return data
