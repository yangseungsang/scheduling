...
데이터 흐름:
    Procedure Service → 실행 목록/상세 데이터 조합
    ExecutionRepository → 실행 상태 읽기/쓰기
"""
...
def execution_list():
    ...
    return jsonify(procedure_service.execution_items(date_filter, location_filter))
...
def total_count(identifier_id):
    ...
    return jsonify({
        'total_count': procedure_service.total_count(
            identifier_id,
    ...
    if current_user:
        for ex in ExecutionRepository.get_all():
            ...
            if performer == current_user:
                return jsonify({
                    'error': '이미 진행 중인 시험이 있습니다.',
                    'code': 'user_busy',
                }), 409

    total = procedure_service.total_count(identifier_id, task_id)
